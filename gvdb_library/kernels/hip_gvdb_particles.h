												 ...
												 
{
	...
	uint3 vox = Get3DPixelJob(blockIdx, blockDim, threadIdx);
	if ( vox.y >= 1  ) return;
										  ...
										  float3* tbuf )							
{
	...
	uint3 t = Get3DPixelJob(blockIdx, blockDim, threadIdx);
	...
	for (n=boff[b]; n < boff[b]+bcnt[b]; n++ ) {
		...
		norm.x = __fadd_rn(__fmul_rn(v2.y, v1.z), -__fmul_rn(v2.z, v1.y)); // U = DOT(CROSS(v2, v1), D)  norm.x <=> U
		norm.y = __fadd_rn(__fmul_rn(v0.y, v2.z), -__fmul_rn(v0.z, v2.y)); // V = DOT(CROSS(v0, v2), D)  norm.y <=> V
		norm.z = __fadd_rn(__fmul_rn(v1.y, v0.z), -__fmul_rn(v1.z, v0.y)); // W = DOT(CROSS(v1, v0), D)  norm.z <=> W
		...
		if ((norm.x >= 0.0f && norm.y >= 0.0f && norm.z >= 0.0f) || (norm.x <= 0.0f && norm.y <= 0.0f && norm.z <= 0.0f)) {
			...
			rad = __fmul_rn(norm.x, v0.x) + __fmul_rn(norm.y, v1.x) + __fmul_rn(norm.z, v2.x); // rad <=> T
