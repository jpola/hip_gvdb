...
__device__ void GetVoxelIndicesPacked(VDBInfo* gvdb, uint3& localIdx, uint3& atlasIdx) {
	...
	localIdx = make_uint3(EXPAND3(threadIdx)) + atlasApron;

	// What atlasIdx would be if atlas_apron were 0
	const uint3 packedVox = Get3DPixelJob(blockIdx, blockDim, threadIdx);
...
#define GVDB_VOXUNPACKED \
	uint3 localIdx = make_uint3(EXPAND3(threadIdx)) + make_uint3(1, 1, 1); \
	uint3 atlasIdx = Get3DPixelJob(blockIdx, blockDim, localIdx); \
	if (atlasIdx.x >= atlasRes.x || atlasIdx.y >= atlasRes.y || atlasIdx.z >= atlasRes.z) return;
...
extern "C" __global__ void gvdbReduction( VDBInfo* gvdb, int3 res, uchar chan, int3 packres, float* outbuf)
{
	...
	uint3 packvox = make_uint3(EXPAND3(blockIdx)) * make_uint3((uint)blockDim.x, (uint)blockDim.y, 1u) + make_uint3(EXPAND3(threadIdx));
