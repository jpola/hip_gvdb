...
#include "hip_gvdb_scene.h"		// GVDB Scene  
#include "hip_gvdb_nodes.h"		// GVDB Node structure 
#include "hip_gvdb_geom.h"		// GVDB Geom helpers
#include "hip_gvdb_dda.h"		// GVDB DDA 
#include "hip_gvdb_raycast.h"	// GVDB Raycasting   
//----------------------------------------- 
   
// Operator functions
#include "hip_gvdb_operators.h"

// Particle functions 
#include "hip_gvdb_particles.h"
	...
	if ( t > 0 ) {																		// yes..
		...
		if ( node != 0x0 ) {															
			...
			if ( gvdb->clr_chan != CHAN_UNDEF ) {										
				...
				clr *= make_float4( make_float3( getColor(gvdb, gvdb->clr_chan, p) ), 1.0f );
			}
		} else {
