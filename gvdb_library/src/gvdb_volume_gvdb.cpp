////
//  hip version of gvdb_volume_gvdb.cpp
//
...
#define HIP_GVDB_MODULE_PTX "hip_gvdb_module.hsaco"
...
#define PUSH_CTX		hipCtxPushCurrent(mContext);
#define POP_CTX			hipCtx_t pctx; hipCtxPopCurrent(&pctx);
#define	MRES	2048
...
VolumeGVDB::VolumeGVDB ()
{
	...
	for (int n=0; n < 5; n++ ) cuModule[n] = (hipModule_t) -1;
	for (int n=0; n < MAX_FUNC; n++ ) cuFunc[n] = (hipFunction_t) -1;
	for (int n=0; n < MAX_CHANNEL; n++ ) { mVDBInfo.volIn[n] = ID_UNDEFINED_TEX; mVDBInfo.volOut[n] = ID_UNDEFINED_SURF; }
...
VolumeGVDB::~VolumeGVDB()
{
	...
	// Delete CUDA objects
	if (cuVDBInfo != 0) {
		cudaCheck(hipFree(cuVDBInfo), "VolumeGVDB", "~VolumeGVDB", "hipFree", "cuVDBInfo", mbDebug);
	}

	for (int n = 0; n < sizeof(cuModule)/sizeof(cuModule[0]); n++) {
		if (cuModule[n] != (hipModule_t)(-1)) {
			cudaCheck(hipModuleUnload(cuModule[n]), "VolumeGVDB", "~VolumeGVDB", "hipModuleUnload", "cuModule[n]", mbDebug);
		}
	}
...
void VolumeGVDB::LoadFunction ( int fid, std::string func, int mid, std::string ptx )
{
	...
	if ( cuModule[mid] == (hipModule_t) -1 ) 
		cudaCheck ( hipModuleLoad ( &cuModule[mid], cptx ), "VolumeGVDB", "LoadFunction", "hipModuleLoad", cptx, mbDebug);
	if ( cuFunc[fid] == (hipFunction_t) -1 )
		cudaCheck ( hipModuleGetFunction ( &cuFunc[fid], cuModule[mid], cfn ), "VolumeGVDB", "LoadFunction", "hipModuleGetFunction", cfn, mbDebug);
}

// Set the current CUDA device, load all GVDB kernels
void VolumeGVDB::SetCudaDevice ( int devid, hipCtx_t ctx )
{
	size_t len = 0;
	...
	//--- Load cuda kernels
	// Raytracing
	LoadFunction ( FUNC_RAYDEEP,			"gvdbRayDeep",					MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_RAYVOXEL,			"gvdbRaySurfaceVoxel",			MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_RAYTRILINEAR,		"gvdbRaySurfaceTrilinear",		MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_RAYTRICUBIC,		"gvdbRaySurfaceTricubic",		MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_RAYSURFACE_DEPTH,	"gvdbRaySurfaceDepth",			MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_RAYLEVELSET,		"gvdbRayLevelSet",				MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_EMPTYSKIP,			"gvdbRayEmptySkip",				MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_SECTION2D,			"gvdbSection2D",				MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_SECTION3D,			"gvdbSection3D",				MODL_PRIMARY, HIP_GVDB_MODULE_PTX );	
	LoadFunction ( FUNC_RAYTRACE,			"gvdbRaytrace",					MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	
	// Sorting / Points / Triangles
	LoadFunction ( FUNC_PREFIXSUM,			"prefixSum",					MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_PREFIXFIXUP,		"prefixFixup",					MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_INSERT_POINTS,		"gvdbInsertPoints",				MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_SORT_POINTS,		"gvdbSortPoints",				MODL_PRIMARY, HIP_GVDB_MODULE_PTX );	
	LoadFunction ( FUNC_SCATTER_DENSITY,	"gvdbScatterPointDensity",		MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_SCATTER_AVG_COL,	"gvdbScatterPointAvgCol",		MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_INSERT_TRIS,		"gvdbInsertTriangles",			MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_SORT_TRIS,			"gvdbSortTriangles",			MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_VOXELIZE,			"gvdbVoxelize",					MODL_PRIMARY, HIP_GVDB_MODULE_PTX );	
	LoadFunction ( FUNC_RESAMPLE,			"gvdbResample",					MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_REDUCTION,			"gvdbReduction",				MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_DOWNSAMPLE,			"gvdbDownsample",				MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_SCALE_PNT_POS,		"gvdbScalePntPos",				MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_CONV_AND_XFORM,		"gvdbConvAndTransform",			MODL_PRIMARY, HIP_GVDB_MODULE_PTX );

	LoadFunction ( FUNC_ADD_SUPPORT_VOXEL,	"gvdbAddSupportVoxel",			MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_INSERT_SUPPORT_POINTS, "gvdbInsertSupportPoints",	MODL_PRIMARY, HIP_GVDB_MODULE_PTX );

	// Topology
	LoadFunction ( FUNC_FIND_ACTIV_BRICKS,	"gvdbFindActivBricks",			MODL_PRIMARY, HIP_GVDB_MODULE_PTX );	
	LoadFunction ( FUNC_BITONIC_SORT,		"gvdbBitonicSort",				MODL_PRIMARY, HIP_GVDB_MODULE_PTX );	
	LoadFunction ( FUNC_CALC_BRICK_ID,		"gvdbCalcBrickId",				MODL_PRIMARY, HIP_GVDB_MODULE_PTX );	
	LoadFunction ( FUNC_FIND_UNIQUE,		"gvdbFindUnique",				MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_COMPACT_UNIQUE,		"gvdbCompactUnique",			MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_LINK_BRICKS,		"gvdbLinkBricks",				MODL_PRIMARY, HIP_GVDB_MODULE_PTX );

	// Incremental Topology
	LoadFunction ( FUNC_CALC_EXTRA_BRICK_ID,"gvdbCalcExtraBrickId",			MODL_PRIMARY, HIP_GVDB_MODULE_PTX );	

	LoadFunction ( FUNC_CALC_INCRE_BRICK_ID,"gvdbCalcIncreBrickId",			MODL_PRIMARY, HIP_GVDB_MODULE_PTX );	
	LoadFunction ( FUNC_CALC_INCRE_EXTRA_BRICK_ID,"gvdbCalcIncreExtraBrickId",			MODL_PRIMARY, HIP_GVDB_MODULE_PTX );	

	LoadFunction ( FUNC_DELINK_LEAF_BRICKS,	"gvdbDelinkLeafBricks",			MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_DELINK_BRICKS,		"gvdbDelinkBricks",				MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_MARK_LEAF_NODE,		"gvdbMarkLeafNode",				MODL_PRIMARY, HIP_GVDB_MODULE_PTX );

	// Gathering
	LoadFunction ( FUNC_COUNT_SUBCELL,		"gvdbCountSubcell",				MODL_PRIMARY, HIP_GVDB_MODULE_PTX );	
	LoadFunction ( FUNC_INSERT_SUBCELL,		"gvdbInsertSubcell",			MODL_PRIMARY, HIP_GVDB_MODULE_PTX );	
	LoadFunction ( FUNC_INSERT_SUBCELL_FP16,"gvdbInsertSubcell_fp16",		MODL_PRIMARY, HIP_GVDB_MODULE_PTX);
	LoadFunction ( FUNC_GATHER_DENSITY,		"gvdbGatherDensity",			MODL_PRIMARY, HIP_GVDB_MODULE_PTX );	
	LoadFunction ( FUNC_GATHER_LEVELSET,	"gvdbGatherLevelSet",			MODL_PRIMARY, HIP_GVDB_MODULE_PTX);
	LoadFunction ( FUNC_GATHER_LEVELSET_FP16, "gvdbGatherLevelSet_fp16",    MODL_PRIMARY, HIP_GVDB_MODULE_PTX);
	
	LoadFunction ( FUNC_CALC_SUBCELL_POS,	"gvdbCalcSubcellPos",			MODL_PRIMARY, HIP_GVDB_MODULE_PTX );	
	LoadFunction ( FUNC_MAP_EXTRA_GVDB,		"gvdbMapExtraGVDB",			    MODL_PRIMARY, HIP_GVDB_MODULE_PTX );	
	LoadFunction ( FUNC_SPLIT_POS,			"gvdbSplitPos",					MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_SET_FLAG_SUBCELL,	"gvdbSetFlagSubcell",			MODL_PRIMARY, HIP_GVDB_MODULE_PTX );	

	LoadFunction ( FUNC_READ_GRID_VEL,		"gvdbReadGridVel",				MODL_PRIMARY, HIP_GVDB_MODULE_PTX );	
	LoadFunction ( FUNC_CHECK_VAL,			"gvdbCheckVal",					MODL_PRIMARY, HIP_GVDB_MODULE_PTX );	
	
	// Apron Updates
	LoadFunction ( FUNC_UPDATEAPRON_F,		"gvdbUpdateApronF",				MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_UPDATEAPRON_F4,		"gvdbUpdateApronF4",			MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_UPDATEAPRON_C,		"gvdbUpdateApronC",				MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_UPDATEAPRON_C4,		"gvdbUpdateApronC4",			MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_UPDATEAPRONFACES_F, "gvdbUpdateApronFacesF",		MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	
	// Operators
	LoadFunction ( FUNC_FILL_F,				"gvdbOpFillF",					MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_FILL_C,				"gvdbOpFillC",					MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_FILL_C4,			"gvdbOpFillC4",					MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_SMOOTH,				"gvdbOpSmooth",					MODL_PRIMARY, HIP_GVDB_MODULE_PTX );
	LoadFunction ( FUNC_NOISE,				"gvdbOpNoise",					MODL_PRIMARY, HIP_GVDB_MODULE_PTX );	
	LoadFunction ( FUNC_CLR_EXPAND,			"gvdbOpClrExpand",				MODL_PRIMARY, HIP_GVDB_MODULE_PTX );	
	LoadFunction ( FUNC_EXPANDC,			"gvdbOpExpandC",				MODL_PRIMARY, HIP_GVDB_MODULE_PTX );	
...
void VolumeGVDB::SetModule ( hipModule_t module )
{
	PUSH_CTX
	
	cudaCheck ( hipDeviceSynchronize (), "VolumeGVDB", "SetModule", "hipDeviceSynchronize", "", mbDebug);

	ClearAtlasAccess ();

	size_t len = 0;
	cudaCheck ( hipModuleGetGlobal ( &cuScnInfo, &len,	module, "scn" ),	"VolumeGVDB", "SetModule", "hipModuleGetGlobal", "cuScnInfo", mbDebug);

	cudaCheck ( hipModuleGetGlobal ( &cuXform,  &len,	module, "cxform" ), "VolumeGVDB", "SetModule", "hipModuleGetGlobal", "cuXform", mbDebug);
	cudaCheck ( hipModuleGetGlobal ( &cuDebug,  &len,	module, "cdebug" ), "VolumeGVDB", "SetModule", "hipModuleGetGlobal", "cuDebug", mbDebug);
...
void VolumeGVDB::ClearAtlasAccess ()
{
	...
	for (int chan=0; chan < num_chan; chan++ ) {
		if ( mVDBInfo.volIn[chan] != ID_UNDEFINED_TEX ) {
			cudaCheck ( hipDestroyTextureObject(mVDBInfo.volIn[chan] ), "VolumeGVDB", "ClearAtlasAccess", "cuTexObjectDestroy", "volIn", mbDebug);
			mVDBInfo.volIn[chan] = ID_UNDEFINED_TEX;
		}
		if (mVDBInfo.volOut[chan] != ID_UNDEFINED_SURF ) {
			cudaCheck ( hipDestroySurfaceObject(mVDBInfo.volOut[chan] ), "VolumeGVDB", "ClearAtlasAccess", "cuSurfObjectDestroy", "volOut", mbDebug);
			mVDBInfo.volOut[chan] = ID_UNDEFINED_SURF;
		}
	}
	cudaCheck ( hipDeviceSynchronize (), "VolumeGVDB", "ClearAtlasAccess", "hipDeviceSynchronize", "", mbDebug);
...
{	
	...
	for (int chan=0; chan < num_chan; chan++ ) {	
		...
		HIP_RESOURCE_DESC resDesc {HIP_RESOURCE_TYPE_ARRAY, atlas.garray, 0};
		hipResourceDesc resDescForSurface {hipResourceTypeArray, atlas.garray};
		...
		switch ( atlas.border ) {
		case F_BORDER:
			texDesc.addressMode[0] = HIP_TR_ADDRESS_MODE_BORDER;
			texDesc.addressMode[1] = HIP_TR_ADDRESS_MODE_BORDER;
			texDesc.addressMode[2] = HIP_TR_ADDRESS_MODE_BORDER;		
			break;
		case F_CLAMP:
		...
		if ( mVDBInfo.volIn[chan] != ID_UNDEFINED_TEX ) {
			cudaCheck ( hipTexObjectDestroy ( mVDBInfo.volIn[chan] ), "VolumeGVDB", "SetupAtlasAccess", "cuTexObjectDestroy", chanmsg, mbDebug);
			mVDBInfo.volIn[ chan ] = ID_UNDEFINED_TEX;
		}
		if ( mVDBInfo.volOut[chan] != ID_UNDEFINED_SURF ) {
			cudaCheck ( hipDestroySurfaceObject ( mVDBInfo.volOut[chan] ), "VolumeGVDB", "SetupAtlasAccess", "cuSurfObjectDestroy", chanmsg, mbDebug);
			mVDBInfo.volOut[ chan ] = ID_UNDEFINED_SURF;
		}
						
		cudaCheck ( hipTexObjectCreate ( &mVDBInfo.volIn[chan], &resDesc, &texDesc, NULL ), "VolumeGVDB", "SetupAtlasAccess", "cuTexObjectCreate", chanmsg, mbDebug);
		cudaCheck ( hipCreateSurfaceObject ( &mVDBInfo.volOut[chan], &resDescForSurface ), "VolumeGVDB", "SetupAtlasAccess", "cuSurfObjectCreate", chanmsg, mbDebug);
...
void VolumeGVDB::FindUniqueBrick(int pNumPnts, int pLevDepth, int& pNumUniqueBrick)
{
	...
	cudaCheck ( hipModuleLaunchKernel ( cuFunc[FUNC_FIND_UNIQUE], 
		pblks, 1, 1, threads, 1, 1, 0, NULL, argsFindUnique, NULL ), "VolumeGVDB", "FindUniqueBrick", "cuLaunch", "FUNC_FIND_UNIQUE", mbDebug);

	cudaCheck ( hipMemcpyDtoH ( &pNumUniqueBrick, mAux[AUX_UNIQUE_CNT].gpu, sizeof(int) ), "VolumeGVDB", "FindUniqueBrick", "hipMemcpyDtoH", "AUX_UNIQUE_CNT", mbDebug);
	...
	cudaCheck ( hipModuleLaunchKernel ( cuFunc[FUNC_COMPACT_UNIQUE], 
...
void VolumeGVDB::RadixSortByByte(int pNumPnts, int pLevDepth)
{
	...
	cudaCheck ( hipMemcpyDtoD (mAux[AUX_SORTED_LEVXYZ].gpu, mAux[AUX_BRICK_LEVXYZ].gpu, length * sizeof(unsigned short) ),
		               "VolumeGVDB", "RadixSortByByte", "hipMemcpyDtoD", "AUX_SORTED_LEVXYZ", mbDebug);
...
void VolumeGVDB::ActivateExtraBricksGPU(int pNumPnts, float pRadius, Vector3DF pOrig, int pRootLev, Vector3DI pRootPos)
{
	...
	cudaCheck ( hipModuleLaunchKernel ( cuFunc[FUNC_CALC_EXTRA_BRICK_ID], 
	...
	cudaCheck ( hipMemcpyDtoH ( extraLevCnt, mAux[AUX_LEVEL_CNT].gpu, pRootLev * sizeof(int) ), "VolumeGVDB", "ActivateExtraBricksGPU", "hipMemcpyDtoH", "AUX_LEVEL_CNT", mbDebug);

	unsigned short* extraUniBricks = (unsigned short*) malloc ( numUniqueExtraBrick * 4 * sizeof(unsigned short) );
	cudaCheck ( hipMemcpyDtoH ( extraUniBricks, mAux[AUX_UNIQUE_LEVXYZ].gpu, numUniqueExtraBrick * 4 * sizeof(unsigned short) ), "VolumeGVDB", "ActivateExtraBricksGPU", "hipMemcpyDtoH", "AUX_UNIQUE_LEVXYZ", mbDebug);
	...
	for (int lev = pRootLev-1; lev >= 0; lev--)	
	{
		...
		cudaCheck ( hipModuleLaunchKernel ( cuFunc[FUNC_LINK_BRICKS], 
...
void VolumeGVDB::ActivateIncreBricksGPU(int pNumPnts, float pRadius, Vector3DF pOrig, int pRootLev, Vector3DI pRootPos, bool bAccum)
{
	...
	cudaCheck ( hipModuleLaunchKernel ( cuFunc[FUNC_CALC_INCRE_EXTRA_BRICK_ID], pblks, 1, 1, threads, 1, 1, 0, NULL, argsCalcExtraLevXYZ, NULL ), 
	...
	cudaCheck ( hipMemcpyDtoH ( extraLevCnt, mAux[AUX_LEVEL_CNT].gpu, pRootLev * sizeof(int) ), "VolumeGVDB", "ActivateIncreBricksGPU", "hipMemcpyDtoH", "AUX_LEVEL_CNT", mbDebug);

	unsigned short* extraUniBricks = new unsigned short[numUniqueExtraBrick * 4];
	cudaCheck ( hipMemcpyDtoH ( extraUniBricks, mAux[AUX_UNIQUE_LEVXYZ].gpu, numUniqueExtraBrick * 4 * sizeof(unsigned short) ), "VolumeGVDB", "ActivateIncreBricksGPU", "hipMemcpyDtoH", "AUX_UNIQUE_LEVXYZ", mbDebug);
	...
	{
		void* argsLink[2] = { &cuVDBInfo, &lev};
		cudaCheck ( hipModuleLaunchKernel ( cuFunc[FUNC_LINK_BRICKS], pblks, 1, 1, threads, 1, 1, 0, NULL, argsLink, NULL ), "VolumeGVDB", "ActivateIncreBricksGPU", "cuLaunch", "FUNC_LINK_BRICKS", mbDebug);
	}
	PERF_POP();
	...
	cudaCheck ( hipModuleLaunchKernel ( cuFunc[FUNC_DELINK_LEAF_BRICKS], pblks, 1, 1, threads, 1, 1, 0, NULL, argsDelinkLeaf, NULL ), "VolumeGVDB", "ActivateIncreBricksGPU", "cuLaunch", "FUNC_DELINK_LEAF_BRICKS", mbDebug);
	for (int lev = 1; lev < pRootLev; lev++)				
	{
		void* argsDelink[2] = { &cuVDBInfo, &lev};
		cudaCheck ( hipModuleLaunchKernel ( cuFunc[FUNC_DELINK_BRICKS], pblks, 1, 1, threads, 1, 1, 0, NULL, argsDelink, NULL ), "VolumeGVDB", "ActivateIncreBricksGPU", "cuLaunch", "FUNC_DELINK_BRICKS", mbDebug);
	}
	PERF_POP();
...
void VolumeGVDB::ActivateBricksGPU(int pNumPnts, float pRadius, Vector3DF pOrig, int pRootLev, Vector3DI pRootPos)
{
	...
	//////////////////////////////////////////////////////////////////////////
	cudaCheck ( hipModuleLaunchKernel ( cuFunc[FUNC_CALC_BRICK_ID], 
	...
	cudaCheck ( hipMemcpyDtoH ( dat1, mAux[AUX_BRICK_LEVXYZ].gpu, length * sizeof(unsigned short) ), "Memcpy dat1", "ActivateBricksGPU" );
	...
	cudaCheck ( hipMemcpyDtoH ( levCnt, mAux[AUX_LEVEL_CNT].gpu, pRootLev * sizeof(int) ), "VolumeGVDB", "ActivateBricksGPU", "hipMemcpyDtoH", "AUX_LEVEL_CNT", mbDebug);

	unsigned short* uniBricks = new unsigned short[numUniqueBrick * 4];
	cudaCheck ( hipMemcpyDtoH ( uniBricks, mAux[AUX_UNIQUE_LEVXYZ].gpu, numUniqueBrick * 4 * sizeof(unsigned short) ), "VolumeGVDB", "ActivateBricksGPU", "hipMemcpyDtoH", "AUX_UNIQUE_LEVXYZ", mbDebug);
...
void VolumeGVDB::FindActivBricks(int pLev,  int pRootlev,  int pNumPnts, Vector3DF pOrig, Vector3DI pRootPos)
{
	...
	cudaCheck ( hipModuleLaunchKernel ( cuFunc[FUNC_FIND_ACTIV_BRICKS], pblks, 1, 1, threads, 1, 1, 0, NULL, args, NULL ), 
					"VolumeGVDB", "ActivateBricksGPU", "cuLaunch", "FUNC_FIND_ACTIV_BRICKS", mbDebug);
	
	// retrieve data
	int* brickIdx = new int[pNumPnts];
	cudaCheck ( hipMemcpyDtoH ( brickIdx, mAux[AUX_PBRICKDX].gpu, pNumPnts*sizeof(int) ), "VolumeGVDB", "ActivateBricksGPU", "hipMemcpyDtoH", "AUX_PBRICKDX", mbDebug);

	Vector3DF* sortedPos = new Vector3DF[pNumPnts];
	Vector3DF* pos = new Vector3DF[pNumPnts];
	cudaCheck ( hipMemcpyDtoH ( pos, mAux[AUX_PNTPOS].gpu, pNumPnts*sizeof(Vector3DF) ), "VolumeGVDB", "ActivateBricksGPU", "hipMemcpyDtoH", "AUX_PNTPOS", mbDebug);
...
{
	// This launches a kernel to clear the hipArray *.
...
void VolumeGVDB::Initialize ()
{
	...
	// Create VDB object
	cudaCheck(hipMalloc(&cuVDBInfo, sizeof(VDBInfo)), "VolumeGVDB", "Initialize", "hipMalloc", "cuVDBInfo", mbDebug);
...
void VolumeGVDB::CheckData ( std::string msg, hipDeviceptr_t ptr, int dt, int stride, int cnt )
{
	PUSH_CTX

	char* dat = (char*) malloc ( stride*cnt );
	cudaCheck ( hipMemcpyDtoH ( dat, ptr, stride*cnt ), "VolumeGVDB", "CheckData", "hipMemcpyDtoH", "", mbDebug);
...
int VolumeGVDB::VoxelizeNode ( Node* node, uchar chan, Matrix4F* xform, float bdiv, float val_surf, float val_inside, float vthresh  )
{
	...
	cudaCheck(hipModuleLaunchKernel(cuFunc[FUNC_VOXELIZE], grid.x, grid.y, grid.z, block.x, block.y, block.z, 0, NULL, args, NULL),
	...
	if ( node->mLev==0 ) {
		...
		if ( node->mLev==0 ) {													// Generate voxels in a brick..
			...
			cudaCheck ( hipDeviceSynchronize(), "sync(AtlasCopyData)" );			//   Must sync after CUDA copy
...
void VolumeGVDB::SolidVoxelize ( uchar chan, Model* model, Matrix4F* xform, float val_surf, float val_inside, float vthresh )
{
	...
	cudaCheck ( hipMemcpyHtoD ( cuXform, xform->GetDataF(), sizeof(float)*16), "VolumeGVDB", "SolidVoxelize", "hipMemcpyHtoD", "cuXform", mbDebug );	// Send transform
...
Vector3DI VolumeGVDB::InsertTriangles ( Model* model, Matrix4F* xform, float& ydiv )
{
	...
	cudaCheck( hipModuleLaunchKernel(cuFunc[FUNC_INSERT_TRIS], grid.x, 1, 1, block.x, 1, 1, 0, NULL, args, NULL),
	...
	// Reset grid counts
	cudaCheck ( hipMemsetD8 ( mAux[AUX_GRIDCNT].gpu, 0, sizeof(uint)*ybins ), "VolumeGVDB", "InsertTriangles", "hipMemsetD8", "AUX_GRIDCNT", mbDebug);
	...
	cudaCheck ( hipModuleLaunchKernel ( cuFunc[FUNC_SORT_TRIS], grid.x, 1, 1, block.x, 1, 1, 0, NULL, args2, NULL ), 
...
void VolumeGVDB::SurfaceVoxelizeGL ( uchar chan, Model* model, Matrix4F* xform )
{
	...
	cudaCheck ( hipDeviceSynchronize(), "VolumeGVDB", "SurfaceVoxelizeGL", "hipDeviceSynchronize", "", mbDebug );
...
void VolumeGVDB::PrepareVDB ()
{
	...
	if ( mVDBInfo.update ) {			
		...
		PUSH_CTX
		cudaCheck ( hipMemcpyHtoD ( cuVDBInfo, &mVDBInfo, sizeof(VDBInfo) ), "VolumeGVDB", "PrepareVDB", "hipMemcpyHtoD", "cuVDBInfo", mbDebug);
		POP_CTX
	}		
}
...
{
	PUSH_CTX
	cudaCheck(hipMemcpyDtoH( &mVDBInfo, cuVDBInfo, sizeof(VDBInfo)), "VolumeGVDB", "RetrieveVDB", "hipMemcpyDtoH", "cuVDBInfo", mbDebug);
	POP_CTX
}
void VolumeGVDB::PrepareVDBPartially ()
{
	...
	PUSH_CTX
	cudaCheck ( hipMemcpyHtoD ( cuVDBInfo, &mVDBInfo, sizeof(VDBInfo) ), "VolumeGVDB", "PrepareVDBPartially", "hipMemcpyHtoD", "cuVDBInfo", mbDebug);
	POP_CTX
}
...
void VolumeGVDB::WriteRenderTexGL ( int chan, int glid )
{
  ...
  if (mRenderBuf[chan].garray == 0) {
    ...
    // Cuda-GL interop to get target hipArray *				
  ...
  // into opengl texture as a hipArray *
  hip_Memcpy2D cp = { 0 };
  cp.dstMemoryType = hipMemoryTypeDevice;		// from cuda buffer
  cp.dstDevice = mRenderBuf[chan].gpu;
  cp.srcMemoryType = hipMemoryTypeArray;			// to hipArray * (opengl texture)
  cp.srcArray = mRenderBuf[chan].garray;
  cp.WidthInBytes = width;
  cp.Height = height;

  cudaCheck(hipMemcpyParam2D(&cp), "VolumeGVDB", "WriteRenderTexGL", "hipMemcpyParam2D", "", mbDebug );
...
void VolumeGVDB::ReadRenderTexGL ( int chan, int glid )
{
	...
	// into opengl texture as a hipArray *
	hip_Memcpy2D cp = {0};
	cp.srcMemoryType = hipMemoryTypeDevice;		// from cuda buffer
	cp.srcDevice = mRenderBuf[chan].gpu;
	cp.dstMemoryType = hipMemoryTypeArray;			// to hipArray * (opengl texture)
	cp.dstArray = mRenderBuf[chan].garray;
	cp.WidthInBytes = width;
	cp.Height = height;
	
	cudaCheck ( hipMemcpyParam2D ( &cp ), "VolumeGVDB", "ReadRenderTexGL", "hipMemcpyParam2D", "", mbDebug );
...
void VolumeGVDB::ResizeRenderBuf ( int chan, int width, int height, int byteperpix )
{
	...
	if ( mRenderBuf[chan].gpu != 0x0 ) { 
		cudaCheck ( hipFree ( mRenderBuf[chan].gpu ), "VolumeGVDB", "ResizeRenderBuf", "hipFree", "", mbDebug);
	}
	cudaCheck ( hipMalloc ( &mRenderBuf[chan].gpu, sz ), "VolumeGVDB", "ResizeRenderBuf", "hipMalloc", "", mbDebug);
...
void VolumeGVDB::PrepareRender ( int w, int h, char shading )
{
	...
	// Depth buffer
	mScnInfo.outbuf		= nullptr;			// NOT USED  (was mRenderBuf[0].gpu;)
	int dbuf = getScene()->getDepthBuf();
	mScnInfo.dbuf 		= (dbuf == 255 ? NULL : mRenderBuf[dbuf].gpu);	

	cudaCheck ( hipMemcpyHtoD ( cuScnInfo, &mScnInfo, sizeof(ScnInfo) ), "VolumeGVDB", "PrepareRender", "hipMemcpyHtoD", "cuScnInfo", mbDebug);

	POP_CTX
}

// Render using custom user kernel
void VolumeGVDB::RenderKernel ( hipFunction_t user_kernel, uchar chan, uchar rbuf )
{
	PUSH_CTX
	...
	cudaCheck ( hipModuleLaunchKernel ( user_kernel, grid.x, grid.y, 1, block.x, block.y, 1, 0, NULL, args, NULL ), "VolumeGVDB", "RenderKernel", "cuLaunch", "(user kernel)", mbDebug);
...
void VolumeGVDB::Render ( char shading, uchar chan, uchar rbuf )
{
	...
	if ( shading==SHADE_OFF ) {
		PUSH_CTX
		cudaCheck ( hipMemsetD8 ( mRenderBuf[rbuf].gpu, 0, static_cast<uint64>(width)*static_cast<uint64>(height)*4 ),
			"VolumeGVDB", "Render", "hipMemsetD8", "(no shading)", mbDebug);
		POP_CTX
		return;
	}
	...
	cudaCheck(hipModuleLaunchKernel(cuFunc[kern], grid.x, grid.y, 1, block.x, block.y, 1, 0, NULL, args, NULL),
...
void VolumeGVDB::Raytrace ( DataPtr rays, uchar chan, char shading, int frame, float bias )
{
	...
	cudaCheck ( hipModuleLaunchKernel ( cuFunc[FUNC_RAYTRACE], grid.x, 1, 1, block.x, 1, 1, 0, NULL, args, NULL ), 
...
{ 	
	...
	cudaCheck(hipModuleLaunchKernel(cuFunc[kern], grid.x, grid.y, grid.z, block.x, block.y, block.z, 0, NULL, args, NULL), 
...
void VolumeGVDB::ComputeKernel (hipModule_t user_module, hipFunction_t user_kernel, uchar channel, bool bUpdateApron, bool skipOverAprons)
{
	...
	cudaCheck ( hipModuleLaunchKernel ( user_kernel, grid.x, grid.y, grid.z, block.x, block.y, block.z, 0, NULL, args, NULL ), 
					"VolumeGVDB", "ComputeKernel", "cuLaunch", "(user kernel)", mbDebug);
	
	
	if ( bUpdateApron ) {
		cudaCheck ( hipDeviceSynchronize(), "VolumeGVDB", "ComputeKernel", "cuCtxSyncronize", "", mbDebug);
...
{ 
	...
	for (int n=0; n < num_iterations; n++ ) {
		cudaCheck ( hipModuleLaunchKernel ( cuFunc[effect], grid.x, grid.y, grid.z, block.x, block.y, block.z, 0, mStream, args, NULL ),
...
void VolumeGVDB::DownsampleCPU(Matrix4F xform, Vector3DI in_res, char in_aux, Vector3DI out_res, Vector3DF out_max, char out_aux, Vector3DF inr, Vector3DF outr)
{
	...
	cudaCheck(hipModuleLaunchKernel(cuFunc[FUNC_DOWNSAMPLE], grid.x, grid.y, grid.z, block.x, block.y, block.z, 0, NULL, args, NULL), 
...
Vector3DF VolumeGVDB::Reduction(uchar chan)
{
	...
	cudaCheck(hipModuleLaunchKernel(cuFunc[FUNC_REDUCTION], grid.x, grid.y, grid.z, block.x, block.y, block.z, 0, NULL, args, NULL), 
...
void VolumeGVDB::Resample ( uchar chan, Matrix4F xform, Vector3DI in_res, char in_aux, Vector3DF inr, Vector3DF outr )
{
	...
	cudaCheck ( hipModuleLaunchKernel ( cuFunc[FUNC_RESAMPLE], grid.x, grid.y, grid.z, block.x, block.y, block.z, 0, NULL, args, NULL ), 
...
void VolumeGVDB::PrefixSum ( hipDeviceptr_t inArray, hipDeviceptr_t outArray, int numElem )
{
	...
	cudaCheck ( hipModuleLaunchKernel ( cuFunc[FUNC_PREFIXSUM], blks1, 1, 1, threads, 1, 1, 0, NULL, argsA, NULL ), 
	...
	cudaCheck ( hipModuleLaunchKernel ( cuFunc[FUNC_PREFIXSUM], blks2, 1, 1, threads, 1, 1, 0, NULL, argsB, NULL ), "cuPrefixSumB", "PrefixSum" );
	...
	cudaCheck ( hipModuleLaunchKernel ( cuFunc[FUNC_PREFIXSUM], blks2, 1, 1, threads, 1, 1, 0, NULL, argsB, NULL ), 
	...
	cudaCheck ( hipModuleLaunchKernel ( cuFunc[FUNC_PREFIXSUM], 1, 1, 1, threads, 1, 1, 0, NULL, argsC, NULL ), 
					"VolumeGVDB", "PrefixSum", "cuLaunch", "FUNC_PREFIXSUM (A2,S2->null)", mbDebug);

	void* argsD[3] = { &mAux[AUX_SCAN1].gpu, &mAux[AUX_SCAN2].gpu, &blks1 };
	cudaCheck ( hipModuleLaunchKernel ( cuFunc[FUNC_PREFIXFIXUP], blks2, 1, 1, threads, 1, 1, 0, NULL, argsD, NULL ), 
					"VolumeGVDB", "PrefixSum", "cuLaunch", "FUNC_PREFIXFIXUP (S2->S1)", mbDebug);		
#endif

	void* argsE[3] = { &outArray, &mAux[AUX_SCAN1].gpu, &numElem };
	cudaCheck ( hipModuleLaunchKernel ( cuFunc[FUNC_PREFIXFIXUP], blks1, 1, 1, threads, 1, 1, 0, NULL, argsE, NULL ), 
...
void VolumeGVDB::SetDataGPU ( DataPtr& dat, int cnt, hipDeviceptr_t gptr, int offs, int stride )
{
	dat.lastEle = cnt;
	dat.usedNum = cnt;
	dat.max = cnt;
...
void VolumeGVDB::CleanAux()
{
	...
	for (int id = 0; id < MAX_AUX; id++) {
		...
		if (mAux[id].gpu != 0x0) 
		{	
			cudaCheck((hipFree(mAux[id].gpu)), "VolumeGVDB", "CleanAux", "hipFree", "", mbDebug);
			mAux[id].gpu = 0x0; 
		}
	}
...
void VolumeGVDB::PrepareAux ( int id, int cnt, int stride, bool bZero, bool bCPU )
{
	...
	if ( bZero ) {
		cudaCheck ( hipMemsetD8 ( mAux[id].gpu, 0, mAux[id].size ), "VolumeGVDB", "PrepareAux", "hipMemsetD8", "", mbDebug);
	}
	POP_CTX
}
...
void VolumeGVDB::InsertPointsSubcell( int subcell_size, int num_pnts, float pRadius, Vector3DF trans,  int& pSCPntsLength)
{
	...
	cudaCheck(hipModuleLaunchKernel(cuFunc[FUNC_SET_FLAG_SUBCELL], pblks, 1, 1, threads, 1, 1, 0, NULL, argsSetFlag, NULL),
	...
	cudaCheck ( hipModuleLaunchKernel ( cuFunc[FUNC_COUNT_SUBCELL], pblks, 1, 1, threads, 1, 1, 0, NULL, argsCounting, NULL ),
	...
	cudaCheck ( hipMemcpyDtoH ( sc_cnt, mAux[AUX_SUBCELL_CNT].gpu, numSCell * sizeof(int) ), "InsertPointsSubcell","","","",false );
	int maxSCCnt = 0;
	...
	const size_t offset = (numSCell - 1) * sizeof(int);
	char* base_ptr_aux_subcell = static_cast<char*>(mAux[AUX_SUBCELL_PREFIXSUM].gpu);
	char* base_ptr_aux_cnt = static_cast<char*>(mAux[AUX_SUBCELL_CNT].gpu);

	void* ptr_aux_last = static_cast<void*>(base_ptr_aux_subcell + offset);
	void* ptr_aux_last_cnt = static_cast<void*>(base_ptr_aux_cnt + offset);
	
	cudaCheck ( hipMemcpyDtoH ( &offset_last, ptr_aux_last, sizeof(int) ),
					"VolumeGVDB", "InsertPointsSubcell", "hipMemcpyDtoH", "AUX_SUBCELL_PREFIXSUM", mbDebug);	
	cudaCheck ( hipMemcpyDtoH ( &cnt_last, ptr_aux_last_cnt, sizeof(int) ), 
					"VolumeGVDB", "InsertPointsSubcell", "hipMemcpyDtoH", "AUX_SUBCELL_CNT", mbDebug);	
	...
	cudaCheck ( hipModuleLaunchKernel ( cuFunc[FUNC_INSERT_SUBCELL], pblks, 1, 1, threads, 1, 1, 0, NULL, argsInsert, NULL ), 
	...
	cudaCheck ( hipModuleLaunchKernel ( cuFunc[FUNC_CALC_SUBCELL_POS], pblks, 1, 1, threads, 1, 1, 0, NULL, argsSCPos, NULL ), 
...
void VolumeGVDB::InsertPointsSubcell_FP16(int subcell_size, int num_pnts, float pRadius, Vector3DF trans,  int& pSCPntsLength)
{
	...
	cudaCheck(hipMemcpyDtoH(&tmpa, ptr_aux_last, sizeof(int)), 
						"VolumeGVDB", "InsertPointsSubcell_FP16", "hipMemcpyDtoH", "AUX_SUBCELL_PREFIXSUM", mbDebug);		
	cudaCheck(hipMemcpyDtoH(&tmpb, ptr_aux_last_cnt, sizeof(int)), 
						"VolumeGVDB", "InsertPointsSubcell_FP16", "hipMemcpyDtoH", "AUX_SUBCELL_CNT", mbDebug);
	...
	cudaCheck(hipModuleLaunchKernel(cuFunc[FUNC_INSERT_SUBCELL_FP16], pblks, 1, 1, threads, 1, 1, 0, NULL, argsInsert, NULL),
...
void VolumeGVDB::MapExtraGVDB (int subcell_size)
{
	...
	int pblks = int(numSCellMapping / threads) + 1;
	cudaCheck(hipModuleLaunchKernel(cuFunc[FUNC_MAP_EXTRA_GVDB], pblks, 1, 1, threads, 1, 1, 0, NULL, argsSCPos, NULL), 
...
void VolumeGVDB::InsertPoints ( int num_pnts, Vector3DF trans, bool bPrefix )
{
	...
	cudaCheck ( hipModuleLaunchKernel ( cuFunc[FUNC_INSERT_POINTS], pblks, 1, 1, threads, 1, 1, 0, NULL, args, NULL ), 
	...
	if ( bPrefix ) {
		...
		cudaCheck ( hipModuleLaunchKernel ( cuFunc[FUNC_SORT_POINTS], pblks, 1, 1, threads, 1, 1, 0, NULL, args, NULL ), 
...
{	
	...
	void* args[12] = { &num_pnts, &offset,
		...
		&trans.x };
	cudaCheck ( hipModuleLaunchKernel ( cuFunc[FUNC_INSERT_SUPPORT_POINTS], pblks, 1, 1, threads, 1, 1, 0, NULL, args, NULL ), 
...
void VolumeGVDB::ScalePntPos (int num_pnts, float scale)
{
	...
	cudaCheck ( hipModuleLaunchKernel ( cuFunc[FUNC_SCALE_PNT_POS], pblks, 1, 1, threads, 1, 1, 0, NULL, args, NULL ),
...
void VolumeGVDB::GatherDensity ( int subcell_size, int num_pnts, float radius, Vector3DF trans, int& pSCPntsLength, int chanDensity, int chanClr, bool bAccum )
{
	...
	PUSH_CTX
		cudaCheck(hipModuleLaunchKernel(cuFunc[FUNC_GATHER_DENSITY], numSCell, 1, 1, subcell_size, subcell_size, subcell_size, 0, NULL, args, NULL), 
...
void VolumeGVDB::GatherLevelSet (int subcell_size, int num_pnts, float radius, Vector3DF trans, int& pSCPntsLength, int chanDensity, int chanClr, bool bAccum )
{
	...
	PUSH_CTX
		cudaCheck(hipModuleLaunchKernel(cuFunc[FUNC_GATHER_LEVELSET], numSCell, 1, 1, subcell_size, subcell_size, subcell_size, 0, NULL, args, NULL), 
...
void VolumeGVDB::GatherLevelSet_FP16(int subcell_size, int num_pnts, float radius, Vector3DF trans, int& pSCPntsLength, int chanDensity, int chanClr)
{
	...
	PUSH_CTX
		cudaCheck(hipModuleLaunchKernel(cuFunc[FUNC_GATHER_LEVELSET_FP16], numSCell, 1, 1, subcell_size, subcell_size, subcell_size, 0, NULL, args, NULL), 
...
void VolumeGVDB::GetBoundingBox(int num_pnts, Vector3DF pTrans)
{
	...
	cudaCheck(hipModuleLaunchKernel(cuFunc[FUNC_SPLIT_POS], pblks, 1, 1, threads, 1, 1, 0, NULL, args, NULL),
	...
	for (uint32_t i = 0; i < 6; ++i) {
		...
		char* base_ptr = static_cast<char*>(mAux[AUX_BOUNDING_BOX].gpu);
		void *ptr = static_cast<void*>(base_ptr + offset);
		
		if (i < 3) {
			gvdbDeviceMinElementF(ptr, mAux[axes[i%3]].gpu,num_pnts);
		}
		else {
			gvdbDeviceMaxElementF(ptr, mAux[axes[i % 3]].gpu, num_pnts);
		}
	}
...
void VolumeGVDB::GetMinMaxVel(int num_pnts)
{
	...
	cudaCheck(hipModuleLaunchKernel(cuFunc[FUNC_SPLIT_POS], pblks, 1, 1, threads, 1, 1, 0, NULL, args, NULL), "VolumeGVDB", "GetMinMaxVel", "cuLaunch", "FUNC_SPLIT_POS", mbDebug);
	PERF_POP();
...
void VolumeGVDB::ReadGridVel(int N)
{
	...
	cudaCheck(hipModuleLaunchKernel(cuFunc[FUNC_READ_GRID_VEL], pblks, 1, 1, threads, 1, 1, 0, NULL, args, NULL), "VolumeGVDB", "ReadGridVel", "cuLaunch", "FUNC_READ_GRID_VEL", mbDebug);
...
void VolumeGVDB::CheckVal(float slice, int chanVx, int chanVy, int chanVz, int chanVxOld, int chanVyOld, int chanVzOld)
{
	...
	cudaCheck(hipModuleLaunchKernel(cuFunc[FUNC_CHECK_VAL], grid.x, grid.y, grid.z, block.x, block.y, block.z, 0, NULL, args, NULL), "VolumeGVDB", "CheckVal", "cuLaunch", "FUNC_CHECK_VAL", mbDebug);

	RetrieveData(mAux[AUX_OUT1]);
	RetrieveData(mAux[AUX_OUT2]);

	cudaCheck(hipDeviceSynchronize(), "VolumeGVDB", "CheckVal", "hipDeviceSynchronize", "", mbDebug);
...
void VolumeGVDB::ScatterDensity ( int num_pnts, float radius, float amp, Vector3DF trans, bool expand, bool avgColor )
{
	...
	cudaCheck ( hipModuleLaunchKernel ( cuFunc[FUNC_SCATTER_DENSITY], pblks, 1, 1, threads, 1, 1, 0, NULL, args, NULL ), 
	...
	if (mAux[AUX_PNTCLR].gpu != NULL && avgColor) {
		...
		cudaCheck( hipModuleLaunchKernel (cuFunc[FUNC_SCATTER_AVG_COL], pblks_avgcol, 1, 1, threads_avgcol, 1, 1, 0, NULL, args_avgcol, NULL), 
...
void VolumeGVDB::ConvertAndTransform(DataPtr& psrc, char psrcbits, DataPtr& pdest, char pdestbits, int num_pnts, Vector3DF wMin, Vector3DF wDelta, Vector3DF trans, Vector3DF scal)
{
	...
	cudaCheck(hipModuleLaunchKernel(cuFunc[FUNC_CONV_AND_XFORM], pblks, 1, 1, threads, 1, 1, 0, NULL, args, NULL),
...
void VolumeGVDB::AddSupportVoxel ( int num_pnts, float radius, float offset, float amp, Vector3DF trans, bool expand, bool avgColor )
{
	...
	cudaCheck ( hipModuleLaunchKernel ( cuFunc[FUNC_ADD_SUPPORT_VOXEL], pblks, 1, 1, threads, 1, 1, 0, NULL, args, NULL ),
