...
////
//  hip version of gvdb_volume_gvdb.h
//
...
//-----------------------------------------------------------------------------
	...
	struct ALIGN(16) VDBInfo {
		...
		hipDeviceptr_t		nodelist[MAXLEV]; // GPU pointer to each level's pool group 0 (nodes)
		hipDeviceptr_t		childlist[MAXLEV]; // GPU pointer to each level's pool group 1 (child lists)
		hipDeviceptr_t		atlas_map; // GPU pointer to the atlas map (which maps from atlas to world space)
		...
		hipTextureObject_t volIn[MAX_CHANNEL]; // Texture reference (read plus interpolation) to atlas per channel
		hipSurfaceObject_t volOut[MAX_CHANNEL]; // Surface reference (read and write) to atlas per channel
	};
	...
	struct ALIGN(16) ScnInfo {
		...
		Vector3DF	cutoff;
		Vector3DF	thresh;
		hipDeviceptr_t	transfer;
		hipDeviceptr_t outbuf;
		hipDeviceptr_t dbuf;		
	};
	...
	public:
			...
			// Setup
			void SetCudaDevice ( int devid, hipCtx_t ctx=NULL );
			void Initialize ();			
			void Clear ();	
			...
			void ComputeKernel ( hipModule_t user_module, hipFunction_t user_kernel, uchar channel, bool bUpdateApron, bool skipOverAprons);
			...
			hipDeviceptr_t getCUVDBInfo()      { return cuVDBInfo; }
			...
			void SetDataGPU ( DataPtr& dat, int cnt, hipDeviceptr_t gptr, int offs, int stride );
			char* getDataCPU ( DataPtr ptr, int n, int stride );
			void PrefixSum ( hipDeviceptr_t outArray, hipDeviceptr_t inArray, int numElements );
			...
			void TimerStart ();
			float TimerStop ();
			void CheckData ( std::string msg, hipDeviceptr_t ptr, int dt, int stride, int cnt );
	...
	protected:
			...
			// Scene 
			ScnInfo			mScnInfo;
			hipDeviceptr_t		cuScnInfo;

			// VDB Data Structure
			VDBInfo			mVDBInfo;			
			hipDeviceptr_t		cuVDBInfo;

			bool			mHasObstacle;
			hipDeviceptr_t		cuOBSVDBInfo; // Non-owning pointer to VDBInfo used for collision

			// CUDA kernels
			hipModule_t		cuModule[5];
			hipFunction_t		cuFunc[ MAX_FUNC ];

			// CUDA pointers		
			hipDeviceptr_t		cuXform;
			hipDeviceptr_t		cuDebug;
