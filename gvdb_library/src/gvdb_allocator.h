...
//-----------------------------------------------------------------------------
	...
	extern void				StartCuda( int devsel, hipCtx_t ctxsel, hipDevice_t& dev, hipCtx_t& ctx, hipStream_t* strm, bool verbose );
	extern GVDB_API bool	cudaCheck ( hipError_t e, const char* obj, const char* method, const char* apicall, const char* arg, bool bDebug);
...
#endif
		hipArray_t		garray;				// gpu array (cuda)
		hipDeviceptr_t	gpu;				// gpu pointer (cuda)	
		hipTextureObject_t tex_obj;			// gpu texture object
		hipSurfaceObject_t surf_obj;			// gpu surface object
	};	
	...
	public:
		...
		hipDeviceptr_t	getPoolGPU ( uchar grp, uchar lev )	{ return mPool[grp][lev].gpu; }
		...
		void	AtlasCopyLinear ( uchar chan, Vector3DI offset, hipDeviceptr_t gpu_buf );
		void	AtlasRetrieveSlice ( uchar chan, int y, int sz, hipDeviceptr_t tempmem, uchar* dest );
		void	AtlasWriteSlice ( uchar chan, int slice, int sz, hipDeviceptr_t gpu_buf, uchar* cpu_src );
		...
		hipDeviceptr_t getAtlasMapGPU(uchar chan) { return (mAtlasMap.size()==0) ? 0 : mAtlasMap[chan].gpu; }
		...
		void SetStream(hipStream_t strm) { mStream = strm;  }
	...
	private:
		...
		hipModule_t					cuAllocatorModule;
		hipFunction_t				cuFillTex;	
		hipFunction_t				cuCopyTexC;
		hipFunction_t				cuCopyTexF;
		hipFunction_t				cuCopyBufToTexC;
		hipFunction_t				cuCopyBufToTexF;
		hipFunction_t				cuCopyTexZYX;
		hipFunction_t				cuRetrieveTexXYZ;
	};
